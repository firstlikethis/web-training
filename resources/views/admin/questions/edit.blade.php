...
@section('content')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="question_text" id="question_text" rows="3" 
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">{{ old('question_text', $question->question_text) }}</textarea>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <h3 class="text-xl font-medium text-gray-800 mb-4">ตัวเลือกคำตอบ</h3>
                    ...
                    @forelse($question->answers as $answer)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $answer->is_correct ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $answer->is_correct ? 'ถูก' : 'ผิด' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <button type="button" class="text-blue-600 hover:text-blue-900 mr-3 edit-answer" data-id="{{ $answer->id }}" data-text="{{ $answer->answer_text }}" data-correct="{{ $answer->is_correct ? '1' : '0' }}">แก้ไข</button>
                                
                                <form method="POST" action="{{ route('admin.questions.answers.destroy', $answer) }}" class="inline-block">
                                    @csrf
                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบคำตอบนี้?')">ลบ</button>
                                </form>
                            </td>
                        </tr>
                    @empty
        ...
        <form action="{{ route('admin.questions.answers.store', $question) }}" method="POST" id="add-answer-form">
            ...
            <div class="flex items-center justify-end">
                <button type="submit" class="bg-green-600 text-white py-2 px-4 rounded hover:bg-green-700">เพิ่มคำตอบ</button>
            </div>
        </form>
        ...
        <div id="edit-answer-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
            <div class="bg-white rounded-lg shadow-xl p-6 max-w-lg w-full mx-4">
                <h3 class="text-xl font-medium text-gray-800 mb-4">แก้ไขคำตอบ</h3>
